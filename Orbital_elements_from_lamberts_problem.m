function [Angular_momentum,Inclination,Eccentricity,RAAN,Argument_of_Perigee,True_anomaly,Radial_velocity,Time_period_of_orbit_hrs,Radius_perigee,Radius_apogee,Semimajor_axis,Semilatus_rectum,Eccentric_anomaly,Mean_anomaly,Orbits_in_a_day,state_vector_at_point_1,state_vector_at_point_2] = Orbital_elements_from_lamberts_problem(r1_vec,r2_vec,elapsed_time_hrs,Orbit_type)
% The following code calculates the orbital elements of a body around earth
% from two successive geocentric position vectors who have a elapsed time
% in between them by solving the lamberts problem.
% ALSO GIVES THE GEOCENTRIC STATE VECTOR AT THE START OF OBSERVATION AND AT
% THE END OF OBSERVATION
% REQUIRED INPUTS:
% *CAN INPUT PERIFOCAL COORDINATES*
% r1_vec = km, geocentric position vector for first observation
% r2_vec = km, geocentric position vector for second observation
...
g = A*sqrt(y(z)/mu_earth); % sec, g lagrange coeff
f_dot = ((sqrt((mu_earth))/(r1_mag*r2_mag))*(sqrt(y(z)/Stumpff_C(z))))*((z*Stumpff_S(z))-1); % sec, f_dot lagrange coeff
...
state_vector_at_point_1 = [r1_vec,v1_vec]; % KM AND KM/SEC geocentric state vector at point 1 (start of observation)
state_vector_at_point_2 = [r2_vec,v2_vec]; % KM AND KM/SEC geocentric state vector at point 2 (end of observation)

%% OUTPUT
% invokes the Orbital_elements_from_State_vectors function to find the
% orbital elements of given orbit around earth.
% the anomalies are with respect to first observation
[Angular_momentum,Inclination,Eccentricity,RAAN,Argument_of_Perigee,True_anomaly,Radial_velocity,Time_period_of_orbit_hrs,Radius_perigee,Radius_apogee,Semimajor_axis,Semilatus_rectum,Eccentric_anomaly,Mean_anomaly,Orbits_in_a_day] = Orbital_elements_from_State_vectors(state_vector_at_point_1);
...
%% SUBFUNCTIONS for lamberts problem
    ...
    function F_z = F(z)
        F_z = (((y(z))/(Stumpff_C(z)))^(3/2))*Stumpff_S(z) + A*sqrt(y(z)) - (sqrt(mu_earth))*elapsed_time_sec; % F(z) function of lamberts problem
    end
